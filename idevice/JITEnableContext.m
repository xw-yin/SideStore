...
- (void)initLoggerWithLogPath:(NSString*)logPath loggingEnabled:(bool)loggingEnabled {
    if(loggingEnabled) {
        idevice_init_logger(Info, Info, (char*)logPath.UTF8String);
    } else {
        idevice_init_logger(Warn, Disabled, nil);
    }
}
...
- (RpPairingFileHandle*)getPairingFileWithError:(NSError**)error {
    ...
    NSURL* pairingFileURL = [docPathUrl URLByAppendingPathComponent:@"ALTPairingFile.mobiledevicepairing"];
